...
|window.IsDark|`boolean`|read|Only `Columns UI` `DarkMode` can be detected with `v3.0.x`. Support for detecting `Default UI` `DarkMode` was added in `v3.1.0`.
...
See the [Fonts](../../fonts) page for how font properties can be accessed or modified.
