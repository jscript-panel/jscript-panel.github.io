...
|:octicons-tag-24: 3.3.11|utils.VersionString|`string`|read|
...
|:octicons-tag-24: 3.3.17|error_on_cancel|`boolean`|Default `false`. If set to `true`, you must use try/catch to prevent script errors.|

Returns a `number` which can used as the `colour` in many methods.

## `utils.ConvertToAscii(str)`
:octicons-tag-24: 3.3.16
...
||---|---|---|
||path|`string`|
|:octicons-tag-24: 3.3.34|max_size|Default is `0` which leaves the original untouched.
...
:octicons-tag-24: 3.3.35, not available in 3.3.40. Will be restored in 3.4.0.
...
|:octicons-tag-24: 3.3.29|strip_trailing_periods|`boolean`|Default `false`. Set to `true` if `str` is a folder name.|
...
!!! example
	```js
	utils.Run('notepad');
	utils.Run('https://foobar2000.org');
